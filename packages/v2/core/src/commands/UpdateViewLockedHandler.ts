...
export class UpdateViewLockedResult {
  ...
  static create(table: Table, events: ReadonlyArray<IDomainEvent>): UpdateViewLockedResult {
    return new UpdateViewLockedResult(table, [...events]);
  }
}
  ...
  async handle(
    ...
    return safeTry<UpdateViewLockedResult, DomainError>(async function* () {
      ...
      const spec = TableUpdateViewPropertiesSpec.create([
        { viewId: command.viewId, isLocked: command.isLocked },
      ]);
      ...
      return ok(UpdateViewLockedResult.create(updateResult.table, updateResult.events));
    });
  }
}

...
import { UpdateViewOrderCommand } from '@teable/v2-core';
import type { ICommandBus, IExecutionContext, UpdateViewOrderResult } from '@teable/v2-core';

export const executeUpdateViewOrderCommandEndpoint = async (
  context: IExecutionContext,
  rawInput: unknown,
  commandBus: ICommandBus
): Promise<IUpdateViewPropertyEndpointResult> => {
  const commandResult = UpdateViewOrderCommand.create(rawInput);

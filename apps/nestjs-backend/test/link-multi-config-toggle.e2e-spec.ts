...
describe('Link field multi-config toggle regression (e2e)', () => {
  ...
  it('preserves source links when converting manyOne twoWay to manyMany oneWay with formula lookup titles', async () => {
    ...
    try {
      ...
      const sourceRows = sourceTable.records;
      const firstRecord = sourceRecords.records.find((record) => record.id === sourceRows[0]!.id);
      const secondRecord = sourceRecords.records.find((record) => record.id === sourceRows[1]!.id);
